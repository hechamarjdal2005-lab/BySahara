        ...
        <div className="max-w-4xl mx-auto px-5 py-8">
          ...
          {heroSubtitle && (
            <p className="text-sm max-w-md" style={{ color: '#F7E5CD' }}>
              {isRtl ? heroSubtitle.subtitle_ar : heroSubtitle.subtitle_en}
            </p>
          )}
              ...
              <div className="relative z-10 p-5 flex flex-col h-full gap-4">
                ...
                {leftPanelTitle && (
                  <h2 className="font-serif text-lg font-bold text-white">
                    {isRtl ? leftPanelTitle.title_ar : leftPanelTitle.title_en}
                  </h2>
                )}
                <div className="space-y-3 flex-1">
                  {contactItems.map((item, i) => (
                    <div key={i} className="flex items-start gap-3">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: '#F8D197', color: '#455324' }}>
                        {item.icon}
                      </div>
                      <div>
                ...
                {followUsTitle && (
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-widest mb-2" style={{ color: '#9FA93D' }}>
                      {isRtl ? followUsTitle.title_ar : followUsTitle.title_en}
                    </p>
                    <div className="flex gap-2">
                      {['f', 'in', 'tw'].map((s) => (
                        <a key={s} href="#" className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold transition-colors" style={{ background: 'rgba(255,255,255,0.12)', color: '#F8D197' }} onMouseEnter={(e) => (e.currentTarget.style.background = '#CC8F57')} onMouseLeave={(e) => (e.currentTarget.style.background = 'rgba(255,255,255,0.12)')}>
                          {s.toUpperCase()}
                        </a>
                      ))}
                    </div>
            ...
            <div className="rounded-2xl p-5" style={{ background: '#fff', border: '1.5px solid #F0E4CC' }}>
              {sent ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    {formTitle && (
                      <h2 className="font-serif text-base font-bold" style={{ color: '#455324' }}>
                        {isRtl ? formTitle.title_ar : formTitle.title_en}
                      </h2>
                    )}
                  </div>
                  ...
                  <button type="submit" className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-sm text-white transition-opacity hover:opacity-90 active:scale-95" style={{ background: 'linear-gradient(135deg, #455324, #617131)', boxShadow: '0 4px 14px #45532430' }}>
                    {formButton ? (isRtl ? formButton.placeholder_ar : formButton.placeholder_en) : tr('إرسال', 'Send Message')}
