...
import { ArrowLeft, ArrowRight, ShoppingCart, Tag, Gift, Package2, Users, Truck, ShieldCheck, ChevronDown, ChevronUp } from "lucide-react";
...
export default function PackDetails() {
  ...
  const pack: Pack | undefined = getAllActivePacks().find((p) => p.id === id);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {pack.cooperative_name && (
                <div className="absolute bottom-4 start-4">
                  <span
                    ...
                    style={{ background: "rgba(248,209,151,0.90)", color: "#455324" }}
                  >
                    🤝 {pack.cooperative_name}
                  </span>
                </div>
            ...
            <div className="grid grid-cols-3 gap-3 mt-4">
              {[
                ...
                >
                  ...
                  <span className="text-xs font-medium" style={{ color: "#455324" }}>
                    {item.label}
                  </span>
                </div>
              ))}
          ...
          <div className="flex flex-col gap-5">
            ...
            <div>
              ...
              <h1 className="font-serif text-2xl sm:text-3xl font-bold mb-2" style={{ color: "#455324" }}>
                {displayName}
              </h1>
              {displayDesc && (
            ...
            >
              <div
                ...
                style={{ background: "#FDF6EC", borderBottom: "1px solid #F0E4CC" }}
              >
                <Package2 size={15} style={{ color: "#CC8F57" }} />
                <span className="text-sm font-bold" style={{ color: "#455324" }}>
                  {tr("محتويات الباقة", "What's included")}
                </span>
                <span
                ...
                {visibleItems.map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between">
                    ...
                    <div className="text-end">
                      {item.is_free ? (
                        <p className="text-xs line-through" style={{ color: "#BA8944" }}>
                          {item.original_price} MAD
                        </p>
                      ) : (
                        <p className="text-sm font-semibold" style={{ color: "#455324" }}>
                          {item.pack_price} MAD
                        </p>
                      )}
                    </div>
                ...
                {pack.items.length > 4 && (
                  ...
                  >
                    {showAllItems
                      ...
                      : tr(`عرض الكل (${pack.items.length})`, `Show all (${pack.items.length})`)}
            ...
            <div
              ...
              style={{ background: "#FDF6EC", border: "1px solid #F0E4CC" }}
            >
              <div className="flex justify-between items-center mb-1.5">
                <span className="text-sm" style={{ color: "#763C19" }}>
                  {tr("السعر الأصلي", "Original price")}
                </span>
                <span className="text-sm line-through" style={{ color: "#BA8944" }}>
                  {pack.total_original_price} MAD
                </span>
              </div>
              <div className="flex justify-between items-center mb-1.5">
                <span className="text-sm font-semibold" style={{ color: "#617131" }}>
                  {tr("توفير", "You save")}
                </span>
                <span className="text-sm font-bold" style={{ color: "#617131" }}>
                  -{pack.savings} MAD
                </span>
              </div>
              <div
                className="flex justify-between items-center pt-2 mt-1"
                style={{ borderTop: "1px dashed #F0E4CC" }}
              >
                <span className="font-bold text-base" style={{ color: "#455324" }}>
                  {tr("سعر الباقة", "Pack price")}
                </span>
                <div>
                  <span className="font-bold text-2xl" style={{ color: "#455324" }}>
                    {pack.pack_price * qty}
                  </span>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <button
                  onClick={() => setQty((q) => Math.min(pack.stock ?? 99, q + 1))}
                  className="w-10 h-11 flex items-center justify-center text-lg font-bold transition-colors"
                  style={{ color: "#455324" }}
                  onMouseEnter={(e) => (e.currentTarget.style.background = "#FDF6EC")}
                  onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}
              ...
              <button
                ...
                style={
                  pack.stock === 0
                    ...
                    : added
                    ? { background: "#617131", color: "#fff" }
                    : { background: "#CC8F57", color: "#fff" }
                }
                onMouseEnter={(e) => {
                  if (pack.stock !== 0 && !added)
                    (e.currentTarget as HTMLElement).style.background = "#b87d4a";
                }}
                onMouseLeave={(e) => {
                  if (pack.stock !== 0 && !added)
              ...
              >
                ...
                {pack.stock === 0
                  ? tr("نفذ المخزون", "Sold Out")
                  : added
                  ? tr("✓ تمت الإضافة", "✓ Added!")
                  : tr("أضف الباقة للسلة", "Add Pack to Cart")}
              </button>
            </div>
