...
const Cooperatives = () => {
  ...
  const getPageSection = (key) => pageContent.find((c) => c.section_key === key);
  ...
  const getProvinceId = (coop) => {
    const provinceKey = lang(coop.province).toLowerCase();
    for (const [id, names] of Object.entries(provinceNames)) {
      if (
        provinceKey.includes(names.en.toLowerCase()) ||
        provinceKey.includes(names.ar.toLowerCase())
      ) {
        return id;
      }
    }
  ...
  const activeProvinces = [
    ...new Set(cooperatives.map((c) => getProvinceId(c)).filter(Boolean)),
  ];
  ...
  const finalFilteredCoops = searchQuery.trim()
    ? filteredCoops.filter((c) => {
        ...
        return (
          lang(c.name).toLowerCase().includes(q) ||
  ...
  const getCoopUrl = (coop) => {
    if (coop.slug && String(coop.slug).trim() !== '') return coop.slug;
    return coop.id;
  };
      ...
      >
        ...
        <div className="relative max-w-5xl mx-auto px-6 py-20 text-center">
          ...
          <h1 className="font-serif text-5xl md:text-6xl font-bold mb-4">
            {heroSection ? lang(heroSection) : tr('التعاونيات', 'Cooperatives')}
          </h1>
          ...
          <p className="text-lg max-w-xl mx-auto" style={{ color: '#F7E5CD' }}>
            {heroSection
              ? isRtl
                ? heroSection.subtitle_ar
                : heroSection.subtitle_en
              : tr('تمكين النساء والحرفيين', 'Empowering women and artisans')}
          </p>
          ...
          <div className="text-center mb-6">
            <div className="flex items-center justify-center gap-2 mb-1">
              ...
              <h2 className="font-serif text-2xl font-bold" style={{ color: '#455324' }}>
                {mapTitleSection
                  ? isRtl
                    ? mapTitleSection.title_ar
                    : mapTitleSection.title_en
            ...
            <p className="text-sm" style={{ color: '#BA8944' }}>
              {mapTitleSection
                ? isRtl
                  ? mapTitleSection.subtitle_ar
                  : mapTitleSection.subtitle_en
          ...
          <div className="text-center mb-6">
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              <h2 className="font-serif text-2xl font-bold" style={{ color: '#455324' }}>
                {searchSection
                  ? isRtl
                    ? searchSection.title_ar
                    : searchSection.title_en
                  : tr('البحث عن تعاونية', 'Search for a Cooperative')}
              </h2>
              <div className="w-1 h-5 rounded-full" style={{ background: '#CC8F57' }} />
            </div>
          </div>
          ...
          <div className="max-w-md mx-auto">
            <div className="relative">
              <input
                ...
                style={{ borderColor: '#F8D197', background: '#fff', color: '#455324' }}
              />
              {searchQuery && (
                <button
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-8 py-12">
        <div className="flex items-center justify-between mb-8">
          <h2 className="font-serif text-2xl font-bold" style={{ color: '#455324' }}>
            {searchQuery.trim()
              ...
              : allCoopsSection
              ? isRtl
                ? allCoopsSection.title_ar
                : allCoopsSection.title_en
        ...
        {finalFilteredCoops.length === 0 ? (
          ...
          >
            ...
            <p style={{ color: '#BA8944' }}>{tr('لا توجد تعاونيات', 'No cooperatives found')}</p>
          </div>
        ) : (
                ...
                >
                  <div className="relative h-32 sm:h-48 overflow-hidden">
                    ...
                    {coop.certifications && coop.certifications.length > 0 && (
                      <div className="absolute top-2 sm:top-3 start-2 sm:start-3 flex gap-1 flex-wrap">
                  ...
                  <div className="p-2.5 sm:p-5">
                    ...
                    <p
                      className="text-xs sm:text-sm leading-relaxed line-clamp-2 mb-2 sm:mb-4"
                      style={{ color: '#763C19' }}
                    >
                      {description}
                    </p>
