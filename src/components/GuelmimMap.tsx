...
const GuelmimMap: React.FC<GuelmimMapProps> = ({
  ...
  const getStyle = (id: string, province: Province) => {
    const isActive = activeProvinces.includes(id);
    ...
    return {
      ...
      stroke: !isActive
        ...
        ? province.color
        : `${province.color}44`,
      strokeWidth: isSelected ? 2 : isHov && isActive ? 1.6 : 1,
      cursor: (isActive ? "pointer" : "default") as React.CSSProperties["cursor"],
      transition: "fill 0.3s ease, stroke 0.25s ease",
      filter: isSelected
        ? `drop-shadow(0 2px 6px ${province.color}55)`
        : isHov && isActive
        ? `drop-shadow(0 1px 4px ${province.color}44)`
        : "none",
    };
  };

  // ── Sidebar Label Card ───────────────────────────────────────────────
  const LabelItem = ({ province, side }: { province: Province; side: "left" | "right" }) => {
      ...
      <div
        ...
        style={{
          ...
          background: isSelected
            ...
            : "#FAF6EE",
          border: isSelected
            ? `2px solid ${province.color}`
            : isHov && isActive
            ? `2px solid ${province.color}66`
            : "2px solid #EDE5D4",
          boxShadow: isSelected
            ? `0 4px 16px ${province.color}28`
            : isHov && isActive
            ? `0 2px 8px ${province.color}18`
            : "none",
      ...
      >
        ...
        <div
          style={{
            ...
            borderRadius: "50%",
            flexShrink: 0,
            background: isSelected ? province.color : isActive ? `${province.color}88` : "#D4C9B0",
            boxShadow: isSelected ? `0 0 0 3px ${province.color}28` : "none",
        ...
        {isSelected && (
          <div
            style={{
              ...
              flexShrink: 0,
              boxShadow: `0 0 0 3px ${province.color}28`,
            }}
          />
        )}
      </div>
  ...
  const renderProvince = (id: string, points: string) => {
    const province = provinceList.find((p) => p.id === id)!;
    const style = getStyle(id, province);
    const isActive = activeProvinces.includes(id);
    const isSelected = selectedProvince === id;
    const isHov = hovered === id;
    ...
    return (
      ...
      >
        <polygon
          ...
          style={{ transition: style.transition, cursor: style.cursor }}
        />
        {isActive && (
          <>
            {/* Province name */}
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          ...
          <rect x="60" y="60" width="160" height="160" fill="transparent" />

          {/* Large → small render order */}
          {renderProvince("assa-zag", ASSA_ZAG)}
          {renderProvince("tan-tan", TAN_TAN)}
          {renderProvince("guelmim", GUELMIM)}
          {renderProvince("sidi-ifni", SIDI_IFNI)}
        </svg>
      </div>
